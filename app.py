...
def validate_email_input(email_text):
    ...
    if not email_text or email_text.strip() == "":
        return False, "Please enter a valid email message!"
    
    if len(email_text.strip()) < 5:
        return False, "Email message too short! Please enter valid email text."
    
    # Check if it contains email characteristics (@ symbol or email-like content)
    email_keywords = ['subject', 'from:', 'to:', 'dear', 'hello', 'email', 'message', 'regards', 'sincerely', 'thanks']
    has_email_keyword = any(keyword in email_text.lower() for keyword in email_keywords)
    has_at_symbol = '@' in email_text
    
    if not (has_email_keyword or has_at_symbol):
        return False, "Please enter a valid email message (with @ or email keywords like 'From', 'To', 'Dear', etc.)"
