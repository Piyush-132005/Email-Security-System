...
def predict():
    ...
    try:
        ...
        phishing_keywords = [
            "verify", "account", "urgent", "suspend",
            "security", "login", "confirm", "password",
            "bank", "update", "click"
        ]

        if prediction == 0:  # model says legitimate
            lower_text = email_text.lower()
            if any(word in lower_text for word in phishing_keywords):
                prediction = 1
